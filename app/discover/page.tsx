...
type PublicRoom = { title: string; owner_name: string | null; status: "registration" | "voting" | "results"; image_url: string | null; votes_cast: number; members: number; joined: boolean; href: string };
...
export default function DiscoverPage() {
  ...
  const runSearch = useCallback(async (searchQuery: string) => {
    ...
    if (!response.ok) setError(translateError(json.error || API_ERROR.ROOM_SEARCH_FAILED)); else setRooms(json);
    setLoading(false);
  }, [translateError]);
  ...
  return <main className="shell wide"><SiteHeader />
    ...
    {loading ? <div className="spinner" /> : rooms.length === 0 ? <div className="empty-state">{t("noPublicRooms")}</div> : <div className="room-grid discover-grid">{rooms.map((room) => <a className="room-card" href={room.href} key={room.href}>
      ...
      <div className="room-body"><span className={`room-status ${room.status}`}>{room.joined ? t("member") : t("open")}</span><h3>{room.title}</h3><p>{t("organizedBy", { name: room.owner_name || t("unknownOrganizer") })}</p><p><Users size={14} /> {t("membersVotes", { members: room.members, votes: room.votes_cast })}</p></div><ArrowRight className="room-arrow" size={19} />
    </a>)}</div>}
  </main>;
}
