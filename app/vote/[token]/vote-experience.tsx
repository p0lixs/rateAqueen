...
function SortableQueen({ queen, index }: { queen: Queen; index: number }) {
   ...
   const {
      ...
      transition,
      isDragging,
   } = useSortable({ id: queen.id });
   return (
      <div
         ref={setNodeRef}
         style={{ transform: CSS.Transform.toString(transform), transition }}
         className={`queen-row ${isDragging ? "dragging" : ""}`}
      >
         <span className="rank">{index + 1}</span>
         {queen.image_url ? (
            <img
               className="queen-photo"
               src={queen.image_url}
               alt={queen.name}
            />
         ) : (
            <span className="queen-photo queen-photo-empty">
               <Crown size={23} />
            </span>
         )}
         ...
         <button
            ...
            aria-label={t("moveQueen", { name: queen.name })}
            {...attributes}
            {...listeners}
         >
...
export default function VoteExperience({ token }: { token: string }) {
   ...
   const load = useCallback(async () => {
      ...
      if (response.status === 401)
         return void (window.location.href = `/auth?next=${encodeURIComponent(`/vote/${token}`)}`);
      if (!response.ok)
         return setError(
            translateError(json.error || API_ERROR.INVALID_INVITATION),
         );
      setData(json);
   ...
   useEffect(() => {
      if (!data || (data.status !== "registration" && !data.voter.has_voted))
         return;
   ...
   async function submit() {
      ...
      const response = await fetch("/api/votes", {
         ...
         body: JSON.stringify({
            token,
            ranking: queens.map((queen) => queen.id),
         }),
      });
      const json = await response.json();
      if (!response.ok) {
      ...
      return (
         <main className="shell center">
            ...
            <section className="hero">
               <p className="eyebrow">{t("resultsTagline")}</p>
               <h2>{t("votingClosed")}</h2>
               <p className="lede">{t("rankingReady")}</p>
               <a className="btn btn-primary" href={`/results/${token}`}>
                  {t("viewRanking")}
               </a>
            </section>
         </main>
      ...
      return (
         <main className="shell center">
            ...
            <section className="hero">
               ...
               <button
                  className="btn btn-primary waiting-button"
                  disabled
                  aria-live="polite"
               >
                  <Hourglass size={17} /> {t("waitingOrganizer")}
               </button>
               <p className="auto-check">{t("autoOpenCheck")}</p>
            </section>
         </main>
      );
   }
         ...
         <main className="shell center">
            ...
            <section className="hero">
               ...
               <button
                  ...
                  aria-live="polite"
               >
                  <Hourglass size={17} /> {t("waitingClose")}
               </button>
               <p className="auto-check">{t("autoCheck")}</p>
            </section>
         </main>
      );
   }
   ...
   return (
      <main className="shell">
         ...
         >
            <SortableContext
               items={queens.map((queen) => queen.id)}
         ...
         <button
            ...
            disabled={sending}
         >
            <Send size={17} /> {sending ? t("sending") : t("confirmRanking")}
         </button>
         <p className="privacy">
            <LockKeyhole size={13} /> {t("privacyVote")}
         </p>
         <ConfirmModal
            open={confirming}
            title={t("submitTitle")}
            description={t("submitText")}
            confirmLabel={t("submitAction")}
            loading={sending}
            onConfirm={submit}
