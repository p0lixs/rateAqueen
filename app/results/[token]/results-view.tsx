...
export default function ResultsView({ token }: { token: string }) {
  ...
  const [data, setData] = useState<{ title: string; owner_name: string | null; votes: number; results: Result[] } | null>(null);
  ...
  useEffect(() => {
    getSupabaseBrowser().auth.getSession().then(({ data: { session } }) => fetch(`/api/results/${token}`, { cache: "no-store", headers: session ? { Authorization: `Bearer ${session.access_token}` } : undefined })).then(async (response) => {
      ...
      if (!response.ok) setError(translateError(json.error || API_ERROR.RESULTS_LOAD_FAILED));
  ...
  return (
    <main className="shell">
      <SiteHeader />
      <section className="vote-head"><p className="eyebrow"><Trophy size={13} /> {t("classification")}</p><h1>{data.title}</h1><p>{t("organizedBy", { name: data.owner_name || t("unknownOrganizer") })}</p><p className="lede">{t("anonymousVotes", { count: data.votes })}</p></section>
