   ...
   if (!membership)
      membership = (
         await supabase
            ...
            .select("token,has_voted")
            .eq("event_id", event.id)
            .eq("device_hash", deviceHash(deviceKey))
            .maybeSingle()
      ).data;
...
export async function POST(
   ...
   const cleanName = usernameFromUser(user || {}) || name?.trim();
   if (!cleanName || cleanName.length > 60)
      return NextResponse.json(
         { error: API_ERROR.INVALID_NAME },
         { status: 400 },
      );
   ...
   if ((count || 0) >= 100)
      return NextResponse.json(
         { error: API_ERROR.PARTICIPANT_LIMIT_REACHED },
         { status: 409 },
      );
   ...
   if (error) {
      ...
      return NextResponse.json(
         { error: API_ERROR.ROOM_JOIN_FAILED },
         { status: 500 },
      );
   }
