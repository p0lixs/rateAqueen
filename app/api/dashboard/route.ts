...
export async function GET(request: Request) {
  ...
  const [{ data: owned, error: ownedError }, { data: invited, error: invitedError }] = await Promise.all([
    supabase.from("events").select("title,status,admin_token,created_at,queens(image_url,sort_order),invitations(has_voted)").eq("owner_id", user.id).order("created_at", { ascending: false }),
    supabase.from("invitations").select("token,has_voted,events(title,status,queens(image_url,sort_order),invitations(has_voted))").eq("user_id", user.id),
  ]);
