...
create or replace function public.submit_anonymous_ballot(p_token text, p_ranking uuid[])
returns text
language plpgsql
security definer
...
revoke all on function public.submit_anonymous_ballot(text, uuid[]) from public, anon, authenticated;
grant execute on function public.submit_anonymous_ballot(text, uuid[]) to service_role;
