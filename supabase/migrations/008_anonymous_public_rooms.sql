-- Ejecuta este archivo UNA VEZ después de 007_registration_phase.sql.
alter table public.invitations add column if not exists device_hash text;

create unique index if not exists invitations_one_device_per_event_idx
on public.invitations(event_id, device_hash)
where device_hash is not null;

-- Las salas públicas aceptan votos anónimos. Las cuentas siguen siendo necesarias
-- para crear y administrar salas, y las restricciones por cuenta existentes se conservan.
